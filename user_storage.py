...
class Users:
    def __init__(self, bucket_name: str = "users_news_data"):
        ...
        self.news_data_bucket: storage.Bucket = self.storage_client.bucket(bucket_name)
    ...
    def get_user(self, chat_id: int) -> User:
        ...
        blob = self.news_data_bucket.blob(f"user_{chat_id}.pickle")  # Отримуємо blob за ключем
        if blob.exists():
